...
export default function CheckoutPage() {
  ...
  function handleAddressContinue() {
    if (!selectedAddressId && !showNewAddr) {
  ...
  async function handlePlaceOrder() {
    ...
    const newOrder = {
      ...
      discount,
      deliveryCharge,
      total: grandTotal + (payMethod === 'cod' ? 50 : 0),
      ...
      {step === 3 && orderSummary && (
        <div className="max-w-lg mx-auto text-center py-10">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between text-gray-600"><span>Subtotal</span><span>{formatPrice(orderSummary.subtotal)}</span></div>
            {orderSummary.discount > 0 && (
              <div className="flex justify-between text-green-600"><span>Discount</span><span>−{formatPrice(orderSummary.discount)}</span></div>
            )}
            <div className="flex justify-between text-gray-600"><span>Delivery</span><span>{orderSummary.deliveryCharge ? formatPrice(orderSummary.deliveryCharge) : 'Free'}</span></div>
            <div className="flex justify-between font-bold text-gray-900 pt-2 border-t border-gray-200"><span>Total paid</span><span>{formatPrice(orderSummary.grandTotal)}</span></div>
          </div>
          ...
          <p className="text-blue-600">
            {orderSummary.delivery === 'express' ? '1–2 business days' : '3–5 business days'}
          </p>
